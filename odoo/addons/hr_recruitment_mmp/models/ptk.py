...
class PTK(models.Model):
    ...
    stage_id = fields.Many2one("hr.recruitment.stage","Stage", required=1, default= lambda self: self.get_stage, tracking=True)
    ...
    type_ptk = fields.Many2one("hr.applicant.refuse.reason","Tipe PTK", domain=[('template_id','=',False)],required=1)
    pendidikan = fields.Selection([("sma","SMA/SMK"),("d3","D3"),("s1","S1"),("s2","S2"),("s3","S3")],"Pendidikan",required=1, tracking=True)
    jurusan = fields.Char("Kualikasi Jurusan", required=1)
    pengalaman = fields.Integer("Pengalaman Kerja", default=0,required=1)
    jml_pengajuan = fields.Integer("Jumlah Pengajuan", default=1, required=1, tracking=True)
    ...
    contract = fields.Selection([('pkwt','PKWT'),('pkwtt','PKWTT')],"Tipe Kontrak", required=1)
    mcu = fields.Selection([('bs','Basic'),('bs_p','Basic Plus'),('prem','Premium')],"Tipe MCU", required=1)
